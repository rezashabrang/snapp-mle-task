...
.PHONY: codestyle
codestyle:
	poetry run pyupgrade --exit-zero-even-if-changed --py312-plus **/*.py
...
check-codestyle:
	...
	poetry run darglint --verbosity 2 snapp_mle_task tests
...
.PHONY: complexity
complexity:
	poetry run radon cc snapp_mle_task --total-average

.PHONY: interrogate
interrogate:
	poetry run interrogate -v snapp_mle_task

.PHONY: maintainability
maintainability:
	poetry run radon mi snapp_mle_task
